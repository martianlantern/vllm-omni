...
class ResidualFSQ(Module):
    ...
    def get_codes_from_indices(self, indices):
        ...
        # REPLACED einx.get_at("q [c] d, b n q -> q b n d", self.codebooks, indices)
        # self.codebooks: (Q, C, D)
        # indices: (B, N, Q)
        
        # Transpose indices to (Q, B, N)
        indices_t = indices.transpose(-1, -2).permute(1, 0, 2)
        
        outputs = []
        for q in range(self.num_quantizers):
             # Gather from codebooks[q] using indices_t[q]
             out_q = F.embedding(indices_t[q], self.codebooks[q]) # (B, N, D)
             outputs.append(out_q)
             
        all_codes = torch.stack(outputs, dim=0) # (Q, B, N, D)
