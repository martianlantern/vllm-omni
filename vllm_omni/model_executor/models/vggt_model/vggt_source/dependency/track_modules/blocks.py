...
class BasicEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        a = _bilinear_interpolate(a, self.stride, H, W)
        b = _bilinear_interpolate(b, self.stride, H, W)
        c = _bilinear_interpolate(c, self.stride, H, W)
        d = _bilinear_interpolate(d, self.stride, H, W)
...
def _bilinear_interpolate(x, stride, h, w):
...
class EfficientUpdateFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.add_space_attn:
            self.virtual_tracks = nn.Parameter(torch.randn(1, num_virtual_tracks, 1, hidden_size))
        else:
            self.virtual_tracks = None
    ...
    def forward(self, input_tensor, mask=None):
        ...
        if self.add_space_attn:
            virtual_tokens = self.virtual_tracks.repeat(B, 1, T, 1)
