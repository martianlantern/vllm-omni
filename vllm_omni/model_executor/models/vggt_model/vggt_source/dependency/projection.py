...
def project_3D_points_np(
    ...
    *,
    ...
    """
    # ----- 0. prep sizes -----------------------------------------------------
    N = points_3d.shape[0]  # #points
    B = extrinsics.shape[0]  # #cameras

    # ----- 1. world → homogeneous -------------------------------------------
    w_h = np.ones((N, 1), dtype=points_3d.dtype)
    points3D_h = np.concatenate([points_3d, w_h], axis=1)  # (N,4)
...
def project_3D_points(points_3d, extrinsics, intrinsics=None, extra_params=None, default=0, only_points_cam=False):
    """
    Transforms 3D points to 2D using extrinsic and intrinsic parameters.
    Args:
        points_3d (torch.Tensor): 3D points of shape Px3.
    ...
    with torch.cuda.amp.autocast(dtype=torch.double):
        # Number of points
        num_points = points_3d.shape[0]  # noqa: F841
        B = extrinsics.shape[0]  # Batch size, i.e., number of cameras
        points3D_homogeneous = torch.cat([points_3d, torch.ones_like(points_3d[..., 0:1])], dim=1)  # Nx4
